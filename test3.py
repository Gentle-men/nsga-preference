...
def minmax(min,max,x,e):
    ...
    return x
ss = MinMaxScaler()
std_data = ss.fit_transform(x1)
for i in range(len(x)):
    x[i][0]=std_data[i][0]
n_folds = 5  # 设置交叉检验的次数
model_br1 = BayesianRidge()  # 建立贝叶斯岭回归模型对象
model_r1 = RandomForestRegressor()  # 建立随机森林回归模型对象
model_lr1 = LinearRegression()  # 建立普通线性回归模型对象
model_svr1 = SVR(kernel='rbf',C=100000)  # 建立支持向量机回归模型对象
# model_svr1 = SVR()  # 建立支持向量机回归模型对象
model_br2 = BayesianRidge()  # 建立贝叶斯岭回归模型对象
model_r2 = RandomForestRegressor()  # 建立随机森林回归模型对象
model_lr2 = LinearRegression()  # 建立普通线性回归模型对象
model_svr2 = SVR()  # 建立支持向量机回归模型对象
model_names = ['BayesianRidge', 'LinearRegression', 'SVR','RandomForest']  # 不同模型的名称列表
model_dic = [model_br1,model_lr1,model_svr1,model_r1]  # 不同回归模型对象的集合
model_dic2 = [model_br2,model_lr2,model_svr2,model_r2]  # 不同回归模型对象的集合
